  ...
  if (client) {
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && blank_line) {
          // Pembacaan sensor juga bisa sampai 2 detik 'lamanya'
          ...
          client.println("<html>");
          client.println("<head></head><body><h1>MAHARIYAH 2 MUHAMMAD HABIBUR RAHMAN - Suhu dan Kelembaban Sensor DHT11</h1><h3>Suhu: ");
          client.println(t);
...
void sendSensorData() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin("http://192.168.91.196:5000/sensor"); // Ganti dengan IP dan port server Anda
